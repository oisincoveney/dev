...
Use `metadata.workflow` JSON with `schema`, `kind`, `state`, `plan`, `approval`, and `runtime`. `plan` is approved scope. `runtime` is mutable execution/audit state. Notes/comments hold verifier output, P3 inline fixes, priority changes, and PR grouping divergence.
