...
# USER resets the grounding count, but ONLY for real user messages — not for
# tool_result echoes, which Claude Code stores as type:"user" events carrying
# {type: "tool_result"} content. Treating those as USER resets the counter on
# every tool call and defeats the gate.
#
# A "real user message" is an event whose content is either a string (legacy /
# slash-command form) or an array containing at least one text/image block.
TOKENS=$(jq -r '
  def real_user:
    ((.message.content // .content)) as $c
    | if ($c | type) == "string" then true
      elif ($c | type) == "array" then
        ([$c[] | select(.type == "text" or .type == "image")] | length) > 0
      else false end;

  if (.type == "user" or .role == "user") then
    if real_user then "USER" else empty end
