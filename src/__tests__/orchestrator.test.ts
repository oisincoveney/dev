...
describe('thin orchestrator', () => {
  ...
  it('renders the harness with mise-managed tool declarations', () => {
    ...
    expect(agents).toContain('except for explicit `/quick --here` P3 work')
    ...
    expect(agents).toContain('`/quick --here` means current-checkout tiny P3 edit')
    ...
    expect(existsSync(join(dir, '.claude/commands/land-prs.md'))).toBe(true)
    expect(existsSync(join(dir, '.claude/commands/pr-daemon.md'))).toBe(false)
    ...
    expect(existsSync(join(dir, '.codex/commands/land-prs.md'))).toBe(true)
    expect(existsSync(join(dir, '.codex/commands/pr-daemon.md'))).toBe(false)
    ...
    expect(existsSync(join(dir, '.opencode/commands/land-prs.md'))).toBe(true)
    expect(existsSync(join(dir, '.opencode/commands/pr-daemon.md'))).toBe(false)
    ...
    expect(readFileSync(join(dir, '.claude/commands/quick.md'), 'utf8')).toContain('Low-ceremony quick task')
    expect(readFileSync(join(dir, '.claude/commands/land-prs.md'), 'utf8')).toContain('bunx @oisincoveney/dev land-prs')
    expect(readFileSync(join(dir, '.opencode/commands/quick.md'), 'utf8')).toContain('Low-ceremony quick task')
    expect(readFileSync(join(dir, '.agents/skills/quick/SKILL.md'), 'utf8')).toContain('Do not edit in the current checkout')
    expect(readFileSync(join(dir, '.agents/skills/quick/SKILL.md'), 'utf8')).toContain('/quick --here')
    expect(readFileSync(join(dir, '.agents/skills/tracker-workflow/SKILL.md'), 'utf8')).toContain('except for explicit `/quick --here` P3 work')
    ...
    expect(bootstrap).toContain('/quick --here means current-checkout tiny P3 edit')
    expect(bootstrap).toContain('/quick means Worktrunk quick worktree')
    expect(bootstrap).toContain('Agent implementation work must use Worktrunk worktrees under .agents/worktrees except explicit /quick --here P3 work.')
