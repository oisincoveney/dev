...
describe('thin orchestrator', () => {
  ...
  it('renders the harness fallback with mise-managed tool declarations', () => {
    ...
    expect(agents).toContain('Current checkout is allowed for answer-only, investigation-only, and explicit `/quick` inline edits')
    ...
    expect(agents).toContain('`/quick` means inline tiny edit')
    ...
    expect(readFileSync(join(dir, '.claude/commands/quick.md'), 'utf8')).toContain('inline current-branch tiny edit')
    expect(readFileSync(join(dir, '.opencode/commands/quick.md'), 'utf8')).toContain('inline current-branch tiny edit')
    expect(readFileSync(join(dir, '.agents/skills/tracker-workflow/SKILL.md'), 'utf8')).toContain('No Worktrunk setup and no delegated implementation agent')
    expect(readFileSync(join(dir, '.agents/skills/tracker-workflow/SKILL.md'), 'utf8')).not.toContain('including `/quick`')
    ...
    expect(bootstrap).toContain('Explicit /quick edits stay inline on the current branch')
